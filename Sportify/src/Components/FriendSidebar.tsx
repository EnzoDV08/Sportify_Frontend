...
const FriendSidebar = ({ className = '', onClose }: { className?: string; onClose?: () => void }) => {
  ...
  const getImageSrc = (path?: string) =>
    path?.startsWith('http')
      ? path
      : path
      ? `${API_BASE}/uploads/${path.replace(/^\/?uploads\//, '')}`
      : '/profile.jpg';
        ...
        {(friends ?? []).map(friend => (
          <div key={friend.id} className="friend-item">
            ...
            <div className="friend-actions">
              <Link to={`/view-profile/${friend.user.userId}`} className="view-btn small-btn">View</Link>
              <button className="remove-btn" onClick={() => handleRemove(friend.id)}><FiTrash2 size={16} /></button>
            </div>
          </div>
        ))}
      </div>
        ...
        {(searchResults ?? []).map(result => {
          ...
          return (
            <div key={result.user.userId} className="search-result">
              <div className="friend-info">
                <img className="friend-avatar" src={getImageSrc(result.profile.profilePicture)} alt={result.user.name} />
              ...
              {isAlreadyFriend ? (
                ...
                <button className="add-btn" onClick={() => handleSendRequest(result.user.userId)}>Send Request</button>
              )}
            </div>
          );
        })}
        ...
        {(requests ?? []).map(req => (
          <div key={req.id} className="request-item">
            ...
            <div className="request-actions">
              <button className="accept-btn" onClick={() => handleAccept(req.id)}><FiCheck size={16} /></button>
              <button className="decline-btn" onClick={() => handleDecline(req.id)}><FiX size={16} /></button>
            </div>
          </div>
        ))}
      </div>
